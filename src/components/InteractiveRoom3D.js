...
const InteractiveMentalHealthSpace = () => {
  ...
  const createNPC = (npcData) => {
    ...
    torso.position.y = TOTAL_HEIGHT * 0.50;
    npc.add(torso);
    ...
    const createDetailedFace = () => {
        ...
        const upperLip = new THREE.Mesh(upperLipGeometry, new THREE.MeshStandardMaterial({
            color: 0xc68075,
            metalness: 0.2,
            roughness: 0.8
        }));
    ...
    const createLimbs = () => {
        ...
        [-1, 1].forEach(side => {
            ...
            const foot = new THREE.Mesh(
                new THREE.BoxGeometry(HEAD_SIZE * 0.35, HEAD_SIZE * 0.25, HEAD_SIZE * 0.8),
                skinMaterial
            );
    ...
    const createNameTag = () => {
        ...
        const bgGeometry = new THREE.PlaneGeometry(HEAD_SIZE * 4, HEAD_SIZE * 0.8);
    ...
    const animate = () => {
        ...
        npc.children.forEach(child => {
            if (child.isGroup && child.position.x !== 0) {
                child.rotation.z = Math.sin(time * 0.7) * 0.03 * Math.sign(child.position.x);
            }
        });
  ...
  useEffect(() => {
    ...
    const animateCharacter = (character, velocity) => {
      ...
      if (walkingSpeed > 0.1) {
        ...
        character.children[5].rotation.x = Math.sin(walkingCycle + Math.PI) * 0.5; // Right leg

        // Arm animations
        character.children[2].rotation.x = Math.sin(walkingCycle + Math.PI) * 0.5; // Left arm
        character.children[3].rotation.x = Math.sin(walkingCycle) * 0.5; // Right arm

        // Torso bounce
        character.children[0].position.y = 1.5 + Math.abs(Math.sin(walkingCycle * 2)) * 0.1;
    ...
    const createEnvironment = () => {
      ...
      const addDecorations = () => {
        ...
        const rockPositions = [
          [-8, -8, 0.5], [8, -8, 0.7], [-8, 8, 0.6], [8, 8, 0.5],
          [-12, -12, 0.8], [12, -12, 0.6], [-12, 12, 0.7], [12, 12, 0.8]
        ];
    ...
    const animate = () => {
      ...
      if (characterRef.current) {
        ...
        if (moveX !== 0 || moveZ !== 0) {
          ...
          // Clamp velocity
          const horizontalVelocity = new THREE.Vector2(body.velocity.x, body.velocity.z);
